...
while {true} do {
    ...
    if ([5] call KPLIB_fnc_hasPermission) then {
        ...
        _nearammoboxes = ((getpos player) nearEntities [JTFV_crates, 10]);
        _neartransporttrucks = ((getpos player) nearEntities [JTFV_transport_classes, 10]);
...

        {
            ...
            if (!(_next_truck in _managed_trucks) && (_truck_load > 0)) then {
                    _action_id = _next_truck addAction ["<t color='#FFFF00'>" + localize "STR_ACTION_UNLOAD_BOX" + "</t>","JTF\scripts\flatbed\do_unload_container.sqf","",-500,true,true,"","build_confirmed == 0 && (_this distance _target < 8) && (vehicle player == player)"];
...

        {
            ...
            if (!(_next_box in _managed_boxes) && ( isNull  attachedTo _next_box )) then {
                _b_action_id1 = _next_box addAction ["<t color='#FFFF00'>" + localize "STR_ACTION_LOAD_BOX" + "</t>",{[_this select 0] call do_load_container;},"",-501,true,true,"","build_confirmed == 0 && (_this distance _target < 5) && (vehicle player == player)"];
